...
jQuery(document).ready(function($){
	...
	//	CsTheme Demo Importer
    var import_btn = jQuery('#run_import');
    if ( import_btn.length > 0 ) {
        import_btn.click( function(e){
            e.preventDefault();

            var answer = confirm( 'Are you sure you want to import the demo data? This may duplicate some content if it exists already.');
            ...
            $.ajax({
                ...
                data:
                {
                    action: 'cstheme_ajax_demo_import',
                    _ajax_nonce: jQuery('input[name=cstheme-demo-import-nonce]').val()
                },
                beforeSend: function()
                {
                    import_btn.fadeOut(300, function(){
                ...
                success: function( response )
                {
                    ...
                    if (
                        response.match( 'demo_data_imported' ) &&
                        response.match( 'cs_pages_set' ) &&
                        response.match( 'cs_menus_set' ) &&
                        response.match( 'cs_permalinks_set' ) )
                    {
                        jQuery('.import-box .import-started-message').fadeOut(300, function(){
                            // jQuery('.import-box .import-success-message').html( '<p>The script returned the following message:</p>' + response );
                    ...
                    else
                    {
                        response = response.
                            ...
                            replace('cs_pages_set','').
                            replace('cs_menus_set','').
                            replace('cs_permalinks_set','').
